...
describe("BrainFeck", () => {
  ...
  describe("Stop", () => {
    ...
    it("should reset data when stopped", () => {
      bf = new BrainFeck(">>+");
      jest.useFakeTimers();
      bf.run();
      jest.runOnlyPendingTimers();
      jest.runOnlyPendingTimers();
      expect(bf.instPointer).toBe(3);
      expect(bf.memPointer).toBe(2);
      expect(bf.memory[2]).toBe(1);
      bf.stop();
      jest.runOnlyPendingTimers();
      expect(bf.currentTimer).toBe(null);
      expect(bf.instPointer).toBe(0);
      expect(bf.memPointer).toBe(0);
      expect(bf.memory[2]).toBe(0);
      jest.useRealTimers();
    });
  });
  describe("Step", () => {
